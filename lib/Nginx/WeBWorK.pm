...
package Nginx::WeBWorK;

=head1 NAME

Nginx::WeBWorK - nginx embedded perl handler for WeBWorK 2.
...
BEGIN {
	...
	if (1) {
		# should something from Nginx be imported here?
	} else {
		require Apache::Log;
		Apache::Log->import;
	}
...
sub main {
	...
	my $result = do {
		...
		eval { WeBWorK::dispatch_webwork($r) };
	};
	...
	if ($@) {
		...
		#unless ($r->bytes_sent) {
		unless ($r->dispatching) {
			$r->header_set("Content-Type" => "text/html");
...
sub error {
	my $r = shift;
	my $error = shift;
	$r->status(Nginx::Simple::HTTP_INTERNAL_SERVER_ERROR);
	$r->print("oh no, there is an error! ($error)");
}
...
sub htmlMessage($$$@) {
	...
	my $method = htmlEscape( $r->request_method );
